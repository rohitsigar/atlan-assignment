...
import { QUERY_SUCCESSFUL, QUERY_UNSUCCESSFUL, RUN_QUERY } from "../../util/constants";
...
function Buttons({
  ...
  const runQuery = () => {
    ...
    if (tableHeaders.length > 0 && tableRows.length > 0) {
      ...
      setCSVData(temp);
      if (temp.length > 0) {
        toast.success(QUERY_SUCCESSFUL);
      } else {
        toast.error(QUERY_UNSUCCESSFUL);
      }
    } else {
      toast.error(QUERY_UNSUCCESSFUL);
    }
    ...
    <div className="flex">
      <div className="p-2">
        <button
          name="reset"
          onClick={reset}
          className="flex items-center justify-center px-4 py-2 mx-auto text-lg text-white bg-yellow-500 border-0 rounded-lg h-11 focus:outline-none hover:bg-yellow-600"
        >
          <svg
      ...
      <div className="p-2">
        ...
        >
          ...
          <div className="font-mono font-bold">{RUN_QUERY}</div>
        </button>
      </div>
    </div>
  );
